...
def check_vectors_valid(VectorList):  # checks vector shape and if it has numbers
    if not isinstance(VectorList, list):
        if isinstance(VectorList, np.ndarray):
...
def close_the_loop(_6Ptool, FPtool, FS6, Fa67, a67, alpha67):
    ...
    FP6Orig = np.sum([FP6OrigTerm1, FP6OrigTerm2, FP6OrigTerm3, FP6OrigTerm4], axis=0)
...
def cylindroid_psi(f, fi, fj, alphaijrad):
    ...
    cos_psi = ((fi / f) * np.sin(alphaijrad) + np.cos(alphaijrad) * sin_psi) / np.sin(
        alphaijrad
    )
...
def vector_to_letter_string(V):
    ...
    for Count, Value in enumerate(V):
        Sign = ""
        if Count > 0:
            if Value >= 0 or Value[0].round(4) == "0.0":
                Sign = "+"
...
def line_to_string(Si, SOLi):
    ...
    return sanitize_string(
        "{ "
        + array_to_string(np.round(Si, 3))
        + " ; "
        + array_to_string(np.round(SOLi, 3))
        + " }"
    )
...
def dyname_to_string(F, M0):
    ...
    return sanitize_string(
        "{ "
        + array_to_string(np.round(F, 3))
        + " ; "
        + array_to_string(np.round(M0, 3))
        + " }"
    )
...
def screw_to_string(Si, SOi=None):
    if SOi is None:
        return sanitize_string(
            "{ "
            + array_to_string(np.round(Si[0:3], 3))
            + " ; "
            + array_to_string(np.round(Si[3:6], 3))
            + " }"
        )
    else:
        ...
        return sanitize_string(
            ...
            + array_to_string(np.round(Si, 3))
            + " ; "
            + array_to_string(np.round(SOi, 3))
            + " }"
        )
